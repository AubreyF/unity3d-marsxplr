...
function LateUpdate () {
	...
	if(Game.Settings.useHypersound == 1) {
		Game.Settings.gameMusic.pitch = Mathf.Clamp(-.5 + Game.Player.rigidbody.velocity.magnitude / 15, .8, 1.5);
	}
	
	//Cursor Locking
	if(Game.Settings.camMode == 0 || Input.GetButton("Fire2") || (Input.GetButton("Snipe") && !Game.Messaging.chatting)) Screen.lockCursor = true;
	...
	//Green Arrow
	var targetPos : Vector3 = transform.InverseTransformPoint(camera.ScreenToWorldPoint(Vector3(camera.pixelWidth - 30 - 65, 30, camera.nearClipPlane + .05)));
	...
	//Chase
		...
		//Agile
			...
			if(Game.Player.transform.gameObject.rigidbody && Game.Player.transform.gameObject.rigidbody.velocity.sqrMagnitude > .1 && Game.Player.transform.gameObject.rigidbody.velocity.normalized.y < .8 && Game.Player.transform.gameObject.rigidbody.velocity.normalized.y > -.8) lastDir = Vector3.Lerp(lastDir,Game.Player.transform.gameObject.rigidbody.velocity.normalized,.1);
		...
		//Arcade
			...
			if(Game.Player.transform.rigidbody.velocity.magnitude > 0) {
			
				var heightDamping = 3.0;
				var distanceDamping = 10.0;
				var rotationDamping = 3.0;
				wantedRotationAngle = Quaternion.LookRotation(Game.Player.transform.rigidbody.velocity).eulerAngles.y;
