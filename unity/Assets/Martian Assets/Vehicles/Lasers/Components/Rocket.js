...
function Start() {
	...
	if(targetVehicle) {
		...
		if(targetVehicle.networkView.isMine) { //Local vehicle, high precision
			var refvel : Vector3 = targetVehicle.gameObject.rigidbody.velocity;
			...
			var vel : Vector3 = targetVehicle.gameObject.rigidbody.velocity;
	...
	else if(lag > 0) rigidbody.position += rigidbody.transform.TransformDirection(0, 0, velocity * lag);	//Position extrapolation for non authoratative firing instances
	
	//Begin Run
	rigidbody.velocity = rigidbody.transform.TransformDirection(0, 0, velocity);
...
function FixedUpdate() {
	...
	if(Game.Settings.laserGrav != 0) rigidbody.velocity.y -= Game.Settings.laserGrav * speedX * Time.deltaTime * 20;
	var hits : RaycastHit[];
    ...
    for (var i=0; i<hits.length; i++) {
		...
		if(((targetVehicle && !targetVehicle.networkView.isMine) || (!targetVehicle && !launchVehicle.networkView.isMine)) && collision.rigidbody) continue;
		
		//We are the instance if this laser on the firer's computer, & we tagged another vehicle
		if(((targetVehicle && targetVehicle.networkView.isMine) || (!targetVehicle && launchVehicle.networkView.isMine)) && collision.rigidbody) {
			...
			if(veh && veh.isResponding) {
				...
				veh.gameObject.networkView.RPC("lH", RPCMode.Others, laserID, collision.transform.InverseTransformPoint(collision.point));
				//We are quarry, and made a tag
				if(launchVehicle.isIt == 1 && (Time.time - veh.lastTag) > 3) {
					launchVehicle.gameObject.networkView.RPC("iS", RPCMode.All, veh.gameObject.name);
					veh.lastTag = Time.time;
				}
				//They were quarry, and we tagged them
				else if(veh.isIt == 1 && (Time.time - launchVehicle.lastTag) > 3 && (Time.time - veh.lastTag) > 3) launchVehicle.gameObject.networkView.RPC("sQ", RPCMode.All, 2);
				//They weren't quarry, and we weren't supposed to shoot them
				else if(veh.isIt == 0 && launchVehicle.isIt == 0) launchVehicle.gameObject.networkView.RPC("dS", RPCMode.All, veh.gameObject.name);
			}
		}
		...
		if(collision.rigidbody || Game.Settings.laserRico == 0) {
			...
			rigidbody.position = collision.point /*+ rigidbody.velocity.normalized * -.1*/;// + collision.normal;
			rigidbody.velocity = Game.Settings.laserRico * Vector3.Lerp(Vector3.Scale(rigidbody.velocity, collision.normal), Vector3.Reflect(rigidbody.velocity, collision.normal), Game.Settings.laserRico);
		}
    }
