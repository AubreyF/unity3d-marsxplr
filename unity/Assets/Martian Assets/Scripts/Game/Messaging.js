...
function ChatWindow (id : int) {
	...
	if (Event.current.type == EventType.keyDown && Event.current.character == "\n" && inputField.Length > 0) {
		if(inputField == "x" || inputField == "/x" || inputField == "/X") {
			...
			Game.Controller.networkView.RPC("msg", RPCMode.Others, inputField + " - " + GameData.userName, parseInt(chatOrigins.Remote));
...
function broadcast(str : String) {
	Game.Controller.networkView.RPC("msg", RPCMode.All, str, parseInt(chatOrigins.Server));
}