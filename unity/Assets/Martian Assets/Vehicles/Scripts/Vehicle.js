...
function Update() {
	...
	if(isBot || !networkView.isMine) {
		if(isIt && markerQuarry && !markerQuarry.active) {
...
function OnGUI() {
	...
	if((networkView.isMine && !isBot) || !Game.Settings.hideNames || (Vector3.Distance(Vector3(pos.x,pos.y,0), Input.mousePosition) < 40 && !Physics.Linecast(transform.position, Camera.main.transform.position, 1 << 8))) {
		if(pos.z > 0 || (networkView.isMine && !isBot)) {
			if(pos.z < 0) pos.z = 0;
			...
			if((pos.z <= 1 || pos.y < sizeY * 1.9) && (networkView.isMine && !isBot)) {
...
function OnPrefsUpdated() {
	...
	if(Game.Settings.ramoSpheres != 0) {
		...
		var tnsor : Vector3 = rigidbody.inertiaTensor;
		var cg : Vector3 = rigidbody.centerOfMass;
		if(!ramoSphere) {
...
function OnCollisionEnter(collision : Collision) {
	if(ramoSphere && ramoSphere.collider.isTrigger == false) ramoSphere.SendMessage("OnCollisionEnter", collision);
}
...
function sT(time : float, info : NetworkMessageInfo) {
	if(!vehicleNet && !networkView.isMine) return;
	...
	//We are authoratative instance, and are being "pinged".
		...
		networkView.RPC("sT", RPCMode.Others, 1.0);
	}
