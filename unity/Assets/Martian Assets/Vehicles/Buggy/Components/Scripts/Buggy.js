...
function InitVehicle(veh : Vehicle) {
	...
	if(vehicle.isPlayer) {
		...
		leftTrail.gameObject.active = rightTrail.gameObject.active = false;
	}
...
function Update() {
	...
	(lod.level == 0 ? wing0 : wing1).gameObject.active = wingOpen;
	//vehicle.inputThrottle = wing.active;
	if(wingOpen) {
...
function FixedUpdate () {
	...
	/*
		//Motor
		motorTorque = Mathf.Max(1,Mathf.Lerp(Game.Settings.buggyPower, 0, Mathf.Abs(motorSpeed) / Game.Settings.buggySpeed) * Mathf.Abs((wing.active ? 0 : vehicle.input.y)));
		...
		for(i=0; i<4; i++) {
			...
			if(Physics.Raycast(pos, transform.up * -1, hit, suspensionRange + wheelRadius, ~1<<4)) {
				...
				vehicle.myRigidbody.AddForceAtPosition(wheels[i].TransformDirection(Vector3.Min(Vector3(
					...
					-velocityAtTouch.y * 30 * (wing.active ? 5 : 1) + compression * 400 * (wing.active && i < 2 ? 8 : 1),		//Shocks, Springs
