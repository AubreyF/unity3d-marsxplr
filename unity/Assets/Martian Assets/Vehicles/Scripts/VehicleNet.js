...
//try {
	...
	if(/*Game.Settings.networkInterpolation > 0 &&*/ vehicle.networkMode == 2 && Time.time > lastPing + pingCheck) {
		if(lastPing < 0) lastPing = Time.time;
			...
			networkView.RPC("sT", RPCMode.All, 0.0);
		}
	}
	
	if(!updatePosition || !states[14] || states[14].t == 0.0 || !vehicle.myRigidbody || !Game.Player || !Game.Player.rigidbody) return;
	...
	else simulatePhysics = (Vector3.Distance(vehicle.myRigidbody.position, Game.Player.rigidbody.position) < 40);
