...
function Update() {
	...
	if(Time.time > syncInpTimer && vehicle.input != inputS) {
		...
		networkView.RPC("s4", RPCMode.Others, Mathf.RoundToInt(inputS.x * 10), Mathf.RoundToInt(inputS.y * 10), Mathf.RoundToInt(inputS.z * 10), Mathf.RoundToInt(inputS.w * 10));
	}
}
...
function OnCollisionStay(collision : Collision) {
	...
	if(collision.transform.parent && collision.transform.parent.gameObject.rigidbody) {
