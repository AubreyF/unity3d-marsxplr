...
function Update () {
	if(Application.loadedLevel == 1 && Lobby.GUIHide > 0) {
		...
		particleEmitter.worldVelocity.y = -Mathf.Min(Time.timeSinceLevelLoad, 5);
		if(Time.timeSinceLevelLoad > 7 || QualitySettings.currentLevel < QualityLevel.Good) particleEmitter.emit = false;
		//particleEmitter.emit = true;
