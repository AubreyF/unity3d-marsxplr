...
function Update () {
	...
	if(vehicle.specialInput) {
		for(var thruster : Transform in hoverThrusters) {
			...
			thruster.gameObject.particleEmitter.localVelocity.y = -1 * vehicle.input.z;
			thruster.gameObject.particleEmitter.minSize = thruster.gameObject.particleEmitter.maxSize = Mathf.Max(.1, vehicle.input.z * .3);
		}
		if(lod.level == 0) {
			mainThrusterParticles.gameObject.particleEmitter.localVelocity.x = 1 * vehicle.input.x;
			mainThrusterParticles.gameObject.particleEmitter.localVelocity.y = -1 * vehicle.input.y;
		}
		...
		if(lod.level == 0) mainThrusterParticles.gameObject.particleEmitter.localVelocity.x = mainThrusterParticles.gameObject.particleEmitter.localVelocity.y = 0;
	...
	mainThrusterParticles.gameObject.particleEmitter.localVelocity.z = Mathf.Min(-10 * (vehicle.specialInput ? .1 : vehicle.input.z), -.5);
	if(mainThrusterParticles.gameObject.particleEmitter.localVelocity.z >= -1) mainThrusterParticles.particleRenderMode = ParticleRenderMode.Billboard;
...
function FixedUpdate() {
	...
	//Landing Gear
	if(vehicle.specialInput) {
		if(!landingGear[lod.level].active) landingGear[lod.level].SetActiveRecursively(true);
		if(landingGearScale < 1) landingGearScale = landingGearScale + Time.deltaTime;
	}
	else {
		if(landingGear[lod.level].active && landingGearScale > 0) landingGearScale = landingGearScale - Time.deltaTime;
		else if(landingGear[lod.level].active) landingGear[lod.level].SetActiveRecursively(false);
	}
	if(landingGear[lod.level].active) landingGear[lod.level].transform.localScale.y = landingGear[lod.level].transform.localScale.x = landingGearScale;
