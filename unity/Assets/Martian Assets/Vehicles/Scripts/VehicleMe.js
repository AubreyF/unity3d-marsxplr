...
function Update () {
	...
	if(Input.GetButtonDown("Jump") && (!Game.Messaging || !Game.Messaging.chatting) && Time.time > Game.Controller.kpTime) {
		networkView.RPC("sI", RPCMode.All, vehicle.specialInput ? false : true);
	...
	if(Input.GetButton("Fire3") && !vehicle.brakes) networkView.RPC("sB", RPCMode.All, true);
	else if(!Input.GetButton("Fire3") && vehicle.brakes) networkView.RPC("sB", RPCMode.All, false);
	...
	if(Game.Settings.laserLocking) {
		...
		vehicle.laserAimer.active = vehicle.laserAimerLocked.active = false;
		laserLock = null;
	}
	...
	if (
		...
		if(laserLock) {
			var snipe = (Game.Settings.firepower[vehicle.vehId] > 2 || (Game.Settings.firepower[vehicle.vehId] > 1 && (laserLock.rigidbody.velocity.sqrMagnitude > 500 || Vector3.Distance(transform.position, laserLock.transform.position) > 500)));
			networkView.RPC((snipe ? "fSl" : "fRl"), RPCMode.All, networkView.viewID, Network.time + "", vehicle.ridePos.position + vehicle.transform.up * -.1, laserLock.networkView.viewID);
		}
		else {
			if(Game.Settings.camMode == 0 || Input.GetButton("Fire2") || Input.GetButton("Snipe")) var rang = camera.main.transform.rotation;
			...
			networkView.RPC((snipe ? "fS" : "fR"), RPCMode.All, networkView.viewID, Network.time + "", vehicle.ridePos.position + vehicle.transform.up * -.1, rang.eulerAngles);
		}
...
function FixedUpdate() {
	if(vehicle.ramoSphere && vehicle.zorbBall) {
		if(vehicle.input.y || vehicle.input.x) {
			rigidbody.AddForce(Vector3.Scale(Vector3(1,0,1), Camera.main.transform.TransformDirection(Vector3(vehicle.input.x * Mathf.Max(0,Game.Settings.zorbSpeed + Game.Settings.zorbAgility),0,vehicle.input.y * Game.Settings.zorbSpeed))), ForceMode.Acceleration);
			rigidbody.AddTorque(Camera.main.transform.TransformDirection(Vector3(vehicle.input.y,0,-vehicle.input.x)) * Game.Settings.zorbSpeed, ForceMode.Acceleration);		
		}
	}
}
