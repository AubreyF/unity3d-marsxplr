...
function Update () {
	...
	if (enemy && Game.Settings.botsCanFire && (Time.time - 1 > rocketFireTime && !Physics.Linecast(transform.position, enemy.transform.position,  1 << 8))) {
		...
		networkView.RPC("fRl", RPCMode.All, networkView.viewID, Network.time + "", vehicle.ridePos.position + vehicle.transform.up * -.1, enemy.networkView.viewID/*Quaternion.LookRotation((enemy.transform.position + vehicle.transform.up * -.1) - vehicle.ridePos.position).eulerAngles*/);
		//var btemp;
